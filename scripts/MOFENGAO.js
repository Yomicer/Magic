...
const giftArray = [
    "MAGIC_COSMIC_DUST","DAMASCUS_STEEL_INGOT","MAGIC_REDSTONE","DAMASCUS_STEEL_INGOT","STEEL_INGOT",
...
function onUse(event) {
    ...
    if (!itemInMainHand || itemInMainHand.getType() !== org.bukkit.Material.DIAMOND_PICKAXE) {
        sendMessage(player, "请不要随意更改工具的材质");
        return;
    }
    ...
    if (itemInOffHand && itemInOffHand.getType() == org.bukkit.Material.DIAMOND ) {
        ...
        if(usageCount <250){
            sendMessage(player, "你的磨粉次数不足250次");
            return;
        }
        sendMessage(player, "镐子自我修复了");
        sendMessage(player, "消耗了一百坤次使用记录以及一颗钻石");
        sendMessage(player, "获得了下面这份礼物");
